...
for (i in 1:num_out){
  ...
  ### Target direction
  tmod_v <- c(-5,5) # set of modification values on target direction
  ...
  if (num_tri%%num_tmod != 0){
    tdir_mod <- c(tdir_mod, rep(5,num_tri%%num_tmod))
  ...
  } else if (cond %in% c("rwd_based","rwd_based_pr")){
  ...
  if (cond %in% c("ga","na","rwd_based","rwd_based_pr","norwd")) {
  ...
  write.table(seq.tgt,sprintf("script/target/%dex%s_%s_%s%s.tgt",blk_tag,sub_tag,fname,cond,sub_tag2),
