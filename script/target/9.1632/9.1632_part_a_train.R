...
tpc <- mean(s_tri_length) + mean(m_tri_length)
...
show_cur_retro_m <- show_cur
show_cur_retro_m[m_tri == 1] <- 0 


seq.tgt_lrn_retro_m <- cbind(field, apply_field, t_radius, t_deg, wait_time, 
                     bval, channel_k11, channel_b11, gain, rot_degree,
                     show_arc, show_cur_retro_m, show_score, train_type_lrn, min_score,
                     max_score, difficulty_lrn, trial_type, blk_phase, task_break)
...
for (blk_tag in blk_tags){
  ...
  write.table(seq.tgt_lrn_retro_m,sprintf("script/target/output/part/%s/%d%s_%s%s_lrn_retro_m_%s.txt",version_id,blk_tag,pre_tag,cond,pos_tag,version_id),
              row.names = F, col.names = F, sep = " ")
