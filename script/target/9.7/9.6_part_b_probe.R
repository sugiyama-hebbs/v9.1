...
set.seed(5) # Set a fix seed so that a sequence can be replicated 14

#### Set key values & parameters ####
## Filename
version_id = "9.6" # version id
...
ph6 = 30 # trials in phase 6 (Post-Probe)
...
rot_pattern <- c(rep(0,pre_genuine),rep(1,10),rep(0,5)) # apply rotation on last 20 trials (10 cycles), whatever precedes them will be "washouts" (no rotation) 
...
plot(t_deg_mod, type ="l", main = cond, xlab = "Trial", ylab = "Rotation [deg]")
