...
blk_tag <- 0 # block tag
cond <- "rwd_based" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
ph8 = 15 # trials in phase 8 (reward-based)
...
rot_degree <- rot_size*c(rep(0,(ph8-10)),rep(1,10)) # hidden rotation for 10 trials. Whatever precedes them are "washouts".
train_type <- rep(3,num_tri)  
...
# output as part of block
write.table(seq.tgt,sprintf("script/target/output/part/%d_%s%s_%s.txt",blk_tag, cond,pos_tag,version_id),
