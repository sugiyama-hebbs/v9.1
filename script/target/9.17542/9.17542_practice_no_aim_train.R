...
blk_tags <- 6 # block tag
cond <- "practice_aim_train" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
s_tri_length <- rep(5,2) # its mean should be an integer
m_tri_length <- rep(5,2) # its mean should be an integer
null_tri_length <- rep(10,2) # its mean should be an integer
...
for (blk_tag in blk_tags){
  write.table(seq.tgt_lrn,sprintf("script/target/output/%s/%d%s_%s%s_lrn_%s.txt",version_id,blk_tag,pre_tag,cond,pos_tag,version_id),
