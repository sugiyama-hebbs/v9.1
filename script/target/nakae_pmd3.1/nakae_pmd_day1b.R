...
version_id = "nakae_pmd3.1" # version id
...
num_sessions <- 12 # number of sessions
num_location <- 3 # number of tms location

s_tri_length <- rep(5,num_location) # its mean should be an integer
m_tri_length <- rep(5,num_location) # its mean should be an integer
null_tri_length <- rep(10,num_location) # its mean should be an integer
...
wait_time <-  (wait_min_pmd3 + ceiling(runif(num_tri,0,addwait_max_pmd3)))/1000 # Wait time

wait_time[s_tri == 1] <- wait_time[s_tri == 1]
wait_time[m_tri == 1] <- wait_time[m_tri == 1]

tms[seq(1,num_tri,2)] <- 1 # This works only if null is an even number. so be careful

if (any(tms == 1 & m_tri == 1)){
  warning("There are M trials with TMS. Make sure this is intended")
}
...
# tms_order_raw <- as.numeric(t(do.call("rbind",lapply(rep(num_location,num_sessions),sample))))
...
even_tri <- rep(1:num_tri,num_sessions/2) + rep(num_tri*seq(1,num_sessions,2), each = num_tri)

for (tms_loc in 1:num_location){
...
seq.tgt_base[even_tri,10] <- seq.tgt_base[even_tri,10]*-1
