...
version_id = "9.272" # version id
blk_tags <- 0 # block tag
cond <- "train_z_0.7" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
rot_pattern <- rep(c(rep(1,5),rep(0,5),rep(-1,5),rep(0,5)),
                   cpb_te_9.27/20) # z ~ 0.1

num_tri <- (spc_te+mpc_te)*cpb_te_9.27
...
show_score[(m_tri == 1 & c(0,rot_degree[1:(num_tri-1)]) != 0) == 1] <- 1
