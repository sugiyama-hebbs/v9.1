...
version_id = "nakae" # version id
blk_tags <- 1 # block tag
cond <- "exp1_constant" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
ph0 = 200 # trials in phase 0 (Pre)
ph1 = 48 # trials in phase 1 (Short perturbation)
ph2 = 8 # trials in phase 2 (No-feedback 1)
ph3 = 144 # trials in phase 3 (Long perturbation)
ph4 = 8 # trials in phase 4 (No-feedback2)
ph5 = 168 # trials in phase 5 (No-feedback3)
ph6 = 0 # trials in phase 6 (Post-Probe)

blk_phase <- c(rep(0,ph0),rep(1,ph1),rep(2,ph2),rep(3,ph3),rep(4,ph4),rep(5,ph5),rep(6,ph6))
num_tri <- ph0 + ph1 + ph2 + ph3 + ph4 + ph5 + ph6
...
show_cur[blk_phase %in% c(2,4,5)] <- 0
rot_degree[blk_phase %in% c(1,3)] <- 30
...
tmod_v <- seq(0,315,45) # set of modification values on target direction
...
wait_time <-  rep(0.5, num_tri) # (wait_min_mod2 + ceiling(runif(num_tri,0,addwait_max_mod3)))/1000 # Wait time
