...
cond <- "probe_multitgt_45deg_no_arc_2" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
s_tri <- rep(c(1,0),ph6/2) # flag s trial
m_tri <- rep(c(0,1),ph6/2)  # flag m trial

rot_pattern <- c(rep(0,(10)),rep(c(1,0),(ph6-10)/2)) # apply rotation on last 20 trials (10 cycles), whatever precedes them will be "washouts" (no rotation) 
...
tmod_v <- seq(-45,45,45) # c(-10,0,10) # set of modification values on target direction
