...
version_id = "9.288" # version id
blk_tags <- 0 # block tag
cond <- "train_z_0.0" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
s_tri <- rep(c(rep(1,spc_te),rep(0,mpc_te)),cpb_te_9.286) # flag s trial
m_tri <- rep(c(rep(0,spc_te),rep(1,mpc_te)),cpb_te_9.286) # flag m trial

rot_pattern <- rep(c(2,-2,2,-6,4,-2,6,-4,6,-4,4,-6),
                   cpb_te_9.286/12) # z ~ 0.1

difficulty_pattern <- abs(1/6*1/2*rep(c(2,-2,2,-6,4,-2,6,-4,6,-4,4,-6),
                          cpb_te_9.286/12)) # z ~ 0.1

num_tri <- (spc_te+mpc_te)*cpb_te_9.286
