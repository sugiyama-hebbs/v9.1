...
set.seed(21) # Set a fix seed so that a sequence can be replicated 6

#### Set key values & parameters ####
## Filename
version_id = "9.1754" # version id
blk_tags <- 0 # block tag
cond <- "probe_part_strategy" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
ph6 = 60 # trials in phase 6 (Post-Probe)
...
pre_rot <- 35 # number of pre-washout trials where genuine target is presented
num_rot_tri <- 10
num_pre_rot_tgt_jump <- 5 # number of trials with target jump that precedes rotation trials

clamp_tri <- (pre_rot+num_rot_tri+1):num_tri # control trials where cursor is clamped
...
rot_pattern <- c(rep(0,pre_rot),rep(1,10),rep(0,15)) # apply rotation on last 20 trials (10 cycles), whatever precedes them will be "washouts" (no rotation) 
...
show_arc[s_tri == 1] <- 7
show_arc[clamp_tri] <- 9
show_arc[(pre_rot-num_pre_rot_tgt_jump+1):(pre_rot+num_rot_tri)] <- 9 # now include preceding washout trials
show_cur[s_tri == 1] <- 2
show_cur[clamp_tri] <- 3
...
tmod_v <- seq(-9,9,3) # c(-10,0,10) # set of modification values on target direction
...
# plot((pre_rot+1):(pre_rot+num_rot_tri), t_deg_mod[(pre_rot+1):(pre_rot+num_rot_tri)], type ="l", main = cond, xlab = "Trial", ylab = "mod direction [deg]")
plot((pre_rot+1):num_tri, t_deg_mod[(pre_rot+1):num_tri], type ="l", main = cond, xlab = "Trial", ylab = "mod direction [deg]")
