...
cond <- "train_mutitgt" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
ph4 = tpc*cpb # trials in phase 4 (Train)
...
# min_score <- rep(-99,num_tri)  
# max_score <- rep(99,num_tri) 
min_score <- rep(0,num_tri)  
max_score <- rep(0,num_tri) 
difficulty <- rep(1,num_tri)

# train_type <- rep(99,num_tri) # This will be replaced, so put some number that is easy to notice in case replacement is not done.
train_type <- rep(0,num_tri) # This will be replaced, so put some number that is easy to notice in case replacement is not done.

s_tri <- rep(c(rep(1,spc),rep(0,mpc)),cpb) # flag s trial
m_tri <- rep(c(rep(0,spc),rep(1,mpc)),cpb) # flag m trial
...
tmod_v <- c(-5,0,5) # set of modification values on target direction
...
t_deg_mod_pre <- rep(tmod_v,floor(sum(m_tri)/num_tmod)) # modify target in M trials only
