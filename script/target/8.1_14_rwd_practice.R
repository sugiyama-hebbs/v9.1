...
# blk_tag <- 18 # block tag
blk_tags <- c(14,22) # block tag
cond <- "rwd_practice" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
ph0 = 30 # trials in phase 0 (Familiarization/Pre-training)
...
# randomly select null trials
null_to_rwd <- 2 # number of null (normal trials) per reward trials
null_tri <- sort(c(1,sample(ph0, floor(ph0*(1-1/(null_to_rwd+1))-1))))

show_score[!1:ph0 %in% null_tri] = 1
show_cur[1:ph0 %in% null_tri] = 2
trial_type[!1:ph0 %in% null_tri] = 4
train_type[!1:ph0 %in% null_tri] = 3 # tag reward_based
