...
cond <- "train_lrn_mid_z" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
cps <- 20 # cycles per one set 
spb <- 3 # sets per block
...
ph4 = tpc*cps # trials in phase 4 (Train)
...
s_tri <- rep(c(rep(1,spc_te),rep(0,mpc_te)),cps) # flag s trial
m_tri <- rep(c(rep(0,spc_te),rep(1,mpc_te)),cps) # flag m trial
...
## Break time
break_time <- rep(0,num_tri)
break_time[num_tri] <- 30 # break at the end of set
...
seq.tgt <- cbind(field, apply_field, t_radius, t_deg, wait_time, 
                 ...
                 max_score, difficulty, trial_type, blk_phase,break_time)
