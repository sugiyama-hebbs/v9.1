...
blk_tag <- 13 # block tag
cond <- "visuorot_multitgt" # condition. Also the main part of filename
pre_tag <- ""
pos_tag <- ""
...
ph0 = 120 # trials in phase 0 (Familiarization/Pre-training)
...
tmod_v <- c(-15,0,15) # set of modification values on target direction
...
rot_degree <-rot_size*c(rep(0,15),rep(1,30), rep(0,30), rep(-1,30),rep(0,15)) # hard code
