...
write.table(seq.tgt_lrn_to_nlrn,sprintf("script/target/output/%s/4_train_lrn_%s.txt",version_id,version_id),
            row.names = F, col.names = F, sep = " ")

write.table(seq.tgt_nlrn_to_lrn,sprintf("script/target/output/%s/6_train_nlrn_%s.txt",version_id,version_id),
            row.names = F, col.names = F, sep = " ")
