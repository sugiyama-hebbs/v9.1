...
set.seed(7) # Set a fix seed so that a sequence can be replicated 14

#### Set key values & parameters ####
## Filename
version_id = "9.152" # version id
...
ph6 = 35 # trials in phase 6 (Post-Probe)
ph7 = 0 # trials in phase 7 (visuomotor)
ph8 = 0 # trials in phase 8 (reward-based)
ph9 = 0 # trials in phase 9 (extra)

pre_genuine <- 15 # number of pre-washout trials where genuine target is presented
...
rot_pattern <- c(rep(0,pre_genuine),rep(1,10),rep(0,10)) # apply rotation on last 20 trials (10 cycles), whatever precedes them will be "washouts" (no rotation) 
...
show_arc[s_tri == 1] <- 7
show_arc[(pre_genuine+1):((pre_genuine+10))] <- 6
show_cur[s_tri == 1] <- 2
...
plot(t_deg_mod, type ="l", main = cond, xlab = "Trial", ylab = "mod direction [deg]")
