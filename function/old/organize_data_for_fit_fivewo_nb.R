...
# This includes 5 washout trials as "initial" trial.
# Only returns with whole set (no first, second, or init)
...
organize_data_for_fit_fivewo_nb <- function(soi,boi, pre_pos = "pos"){
  ...
  source("script/ind_process/lsse_fit_param_fivewo.R")
  ...
  ### Change this to do different target bias alignment
  # tag: tgt bias
  # Get parameter file data
  data_tri_clean = read.csv(sprintf("%s/data_tri_clean_nb_null.csv",data_fpath))
  ...
  data_fit_clean = data_fit %>%
    dplyr::filter(total_tri %in% toi) %>% # Extract Probe trials + preceding washout
    ...
    mutate(seg_num = (row_num-1)%/%seg_len + 1) %>% # segment number
    dplyr::filter(blk %in% boi)
