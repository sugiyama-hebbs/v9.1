...
organize_data_for_fit_seg_nb <- function(soi,boi,set_type = "seg", pre_pos = "pos"){
  ...
  data_tri_clean = read.csv(sprintf("%s/data_tri_clean_nb_prebl.csv",data_fpath)) # Tag. Change this for different bias removal 
  
  data_fit = data_tri_clean %>%
    mutate(phase = data_tgt$phase) %>%
    mutate(shift = data_tgt$shift) %>%
